...
public:
  ...
  EquityPriceGenerator(double initEquityPrice, unsigned numTimeSteps,
      double timeToMaturity, double drift, double volatility);

  /*
    We could also have another ctor that takes in a TermStructure object in
    place of a constant drift or risk free rate, as well as a time path
    determined by a schedule based on dates and a daycount rule; viz,
    EquityPriceGenerator(double initEquityPrice, const RealSchedule&
    realSchedule, const TermStructure& ts, double volatility);
  */
