...
Since $V_t/B_t$ is a martingale, we have $\frac{V_t}{B_t}=\E\left(\frac{\phi(S_T)}{B_T}\middle|\mathcal F_t^S\right)$, or

$$V_t=e^{-(T-t)r}\E\left(S_T^p\middle|\mathcal F_t^S\right)\,.$$

Plugging in $S_T$ yields

$$V_t=S_t^pe^{(T-t)\left(-r+\mu p-\frac{\sigma^2p}2\right)}\E\left(e^{(W_T-W_t)\sigma p}\middle|\mathcal F_t^S\right)\,.$$
...
Since $\frac{W_T-W_t}{\sqrt{T-t}}\sim\mathcal N(0,1)$, we can expand the expectation with the PDF of the standard normal distribution such that

$$V_t=S_t^pe^{(T-t)\left(-r+rp-\frac{\sigma^2p}2\right)}\int_\mathbb Re^{x\sigma p\sqrt{T-t}}
